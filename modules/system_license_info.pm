sub system_license_info
    ...
    if ($service_content->about->apiType eq 'HostAgent')
       ...
       if ($service_content->about->apiType eq 'VirtualCenter')
          ...
          $license_assignment_manager =  Vim::get_view( mo_ref => $vcenter_license_manager->licenseAssignmentManager);
          ...
          if ($host_defined)
             ...
             $assigned_licenses = $license_assignment_manager->QueryAssignedLicenses(entityId => $host_view->get_property('mo_ref')->value,);

             }
          else
             {
             # request vCenter license
             $assigned_licenses = $license_assignment_manager->QueryAssignedLicenses(entityId => $service_content->about->instanceUuid,);
             }
          ...
          $licenses_properties = @{$assigned_licenses}[0]->properties;
    ...
    if ($evaluation_seconds lt 0)
       ...
       if ($license_name eq '')
          ...
          {
          if ($license_name eq 'Evaluation Mode')
             {
